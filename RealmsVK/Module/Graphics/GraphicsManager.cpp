...
private:
	...
	void createSwapChain () {
		...
		if (vkCreateSwapchainKHR (device, &createInfo, nullptr, &(_renderpipeline.swapChain)) != VK_SUCCESS) {
			throw std::runtime_error ("failed to create swap chain!");
		}

		vkGetSwapchainImagesKHR (device, _renderpipeline.swapChain, &imageCount, nullptr);
		_renderpipeline.swapChainImages.resize (imageCount);
		vkGetSwapchainImagesKHR (device, _renderpipeline.swapChain, &imageCount, _renderpipeline.swapChainImages.data ());
	...
	void createImageViews () {
		...
		for (size_t i = 0; i < _renderpipeline.swapChainImages.size (); i++) {
				...
				if (vkCreateImageView (device, &createInfo, nullptr, &(_renderpipeline.swapChainImageViews[i])) != VK_SUCCESS) {
	...
	void createRenderPass () {
		...
		if (vkCreateRenderPass (device, &renderPassInfo, nullptr, &(_renderpipeline.renderPass)) != VK_SUCCESS) {
	...
	void createGraphicsPipeline () {
		...
		if (vkCreatePipelineLayout (device, &pipelineLayoutInfo, nullptr, &(_renderpipeline.pipelineLayout)) != VK_SUCCESS) {
		...
		if (vkCreateGraphicsPipelines (device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &(_renderpipeline.graphicsPipeline)) != VK_SUCCESS) {
	...
	void createFramebuffers () {
		...
		for (size_t i = 0; i < _renderpipeline.swapChainImageViews.size (); i++) {
			...
			if (vkCreateFramebuffer (device, &framebufferInfo, nullptr, &(_renderpipeline.swapChainFramebuffers[i])) != VK_SUCCESS) {
	...
	void drawFrame () {
		...
		uint32_t imageIndex;
		vkAcquireNextImageKHR (device, _renderpipeline.swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
...
void rlms::GraphicsManagerImpl::stop () {
	...
	vkDestroyPipeline (device, _renderpipeline.graphicsPipeline, nullptr);
	vkDestroyPipelineLayout (device, _renderpipeline.pipelineLayout, nullptr);
	vkDestroyRenderPass (device, _renderpipeline.renderPass, nullptr);

	for (auto imageView : _renderpipeline.swapChainImageViews) {
		vkDestroyImageView (device, imageView, nullptr);
	}
	vkDestroySwapchainKHR (device, _renderpipeline.swapChain, nullptr);
