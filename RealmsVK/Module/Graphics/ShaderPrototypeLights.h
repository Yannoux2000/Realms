...
public:
	static inline Shader vertexShader () {
		const char* __vertexShaderSrc = R"glsl(
	#version 330 core
	...
	void main () {
		...
		return Shader (/*GL_VERTEX_SHADER*/ 0, __vertexShaderSrc);
	}

	static inline Shader geometryShader () {
		const char* __geometryShaderSrc = R"glsl(
	#version 330 core
	layout(points) in;
	...
	void main() {
		...
		return Shader (/*GL_GEOMETRY_SHADER*/ 0, __geometryShaderSrc);
	}

	static inline Shader fragmentShader () {
		const char* __fragmentShaderSrc = R"glsl(
	#version 330 core
	...
	vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir, vec4 color) {
		...
		return Shader (/*GL_FRAGMENT_SHADER*/ 0, __fragmentShaderSrc);
	}
};
