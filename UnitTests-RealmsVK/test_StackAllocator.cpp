...
void* TestStackAllocator::large_mem;
void* TestStackAllocator::small_mem;

TEST_F (TestStackAllocator, getStart) {
	data_obj* a = nullptr;
	ASSERT_NE (nullptr, large_mem);

	StackAllocator alloc (large_mem, large_size);
	{
		SCOPED_TRACE ("Init");
...
TEST_F (TestStackAllocator, getSize) {
	data_obj* a = nullptr;
	ASSERT_NE (nullptr, large_mem);
...
TEST_F (TestStackAllocator, getNumAllocations) {
	data_obj* a = nullptr;
	ASSERT_NE (nullptr, large_mem);
...
TEST_F (TestStackAllocator, getUsedMemory) {
	data_obj* a = nullptr;
	ASSERT_NE (nullptr, large_mem);
	...
	{
		SCOPED_TRACE ("Alloc");
		ASSERT_GE (alloc.getUsedMemory (), data_obj_size);
	}
...
TEST_F (TestStackAllocator, data_obj) {
	data_obj* a = nullptr;
